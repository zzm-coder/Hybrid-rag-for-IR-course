...
本文档介绍基于航天标准 PDF 的混合 RAG（Hybrid-RAG）方法的完整处理流程（0–5 步），包括数据抽取、知识图谱构建、文本分片与向量库构建、图谱与向量库映射、QA 数据集生成、以及 Hybrid-RAG 的评估与前端可视化部署。
...
- `4_RAG_method/Mix-RAG-v1/`：Hybrid-RAG 实现、评估器、前端后端示例与结果目录
- `5_Mix-RAG-show/`：前端可视化与 API 示例
...
```bash
conda create -n mix-rag python=3.9 -y
conda activate mix-rag
pip install -r 5_Mix-RAG-show/requirements.txt
...
`4_RAG_method/Mix-RAG-v1/` 包含 Hybrid-RAG 的实现：
...
```bash
# 启动后端（示例）
python 5_Mix-RAG-show/backend_api.py
# 在另一终端启动前端（或直接打开静态 HTML）
python 5_Mix-RAG-show/frontend_app.py
```
...
- RAG 结果：`4_RAG_method/Mix-RAG-v1/results/` 和 `evaluation_results/`
...
6. 在 `4_RAG_method/Mix-RAG-v1` 运行 `main.py` 或调用 `RAGEvaluator` 进行评估。
7. 启动 `5_Mix-RAG-show` 中的后端/前端以做展示。
